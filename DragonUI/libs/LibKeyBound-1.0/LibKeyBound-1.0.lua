...
function LibKeyBound:Initialize()
	do
		...
		getglobal(resetBindings:GetName() .. 'Text'):SetText(L.ResetKeybinds or "Reset Keybinds")

		resetBindings:SetScript('OnClick', function(self)
			if InCombatLockdown() then
				UIErrorsFrame:AddMessage(L.CannotResetInCombat or "Cannot reset keybinds while in combat.", 1, 0.3, 0.3, 1, UIERRORS_HOLD_TIME)
			else
	...
	StaticPopupDialogs["LIBKEYBOUND_RESET_KEYBINDS"] = {
		text = "Reset all key bindings to default?", -- Will be updated in OnShow
		button1 = OKAY or "Okay",
		button2 = CANCEL or "Cancel", 
		OnShow = function(self)
			-- Update text with localized version when dialog is shown
			local L = LibKeyBoundLocale10 or LibKeyBoundBaseLocale10
			if L and L.ResetKeybindsConfirm then
				self.text:SetText(L.ResetKeybindsConfirm)
			end
			-- Update button texts
			if OKAY and self.button1 then
				self.button1:SetText(OKAY)
			end
			if CANCEL and self.button2 then
		...
		OnAccept = function()
			local L = LibKeyBoundLocale10 or LibKeyBoundBaseLocale10
			if InCombatLockdown() then
				UIErrorsFrame:AddMessage(L and L.CannotResetInCombat or "Cannot reset keybinds while in combat.", 1, 0.3, 0.3, 1, UIERRORS_HOLD_TIME)
			else
				-- Get current binding set (1 = account-wide, 2 = character-specific)
				local currentSet = GetCurrentBindingSet()
				local otherSet = (currentSet == 1) and 2 or 1
				
				-- Strategy: Preserve the other set by saving it first, then reset only current set
				-- This prevents LoadBindings(0) from affecting the other set
				
				-- Step 1: Save the other set's bindings to preserve them
				-- Switch to other set and save it (this ensures it's preserved)
				LoadBindings(otherSet)
				SaveBindings(otherSet)
				
				-- Step 2: Switch to current set
				LoadBindings(currentSet)
				
				-- Step 3: Load default bindings (this may affect both sets, but we'll fix that)
				LoadBindings(0)
				
				-- Step 4: Immediately save current set with defaults
				SaveBindings(currentSet)
				
				-- Step 5: Restore the other set (this overwrites any defaults that were loaded into it)
				LoadBindings(otherSet)
				SaveBindings(otherSet)
				
				-- Step 6: Switch back to current set
				LoadBindings(currentSet)
				...
				UIErrorsFrame:AddMessage(L and L.ResetKeybindsSuccess or "All keybinds reset to default.", 0, 1, 0, 1, UIERRORS_HOLD_TIME)
			end
		end,
		OnCancel = function()
			-- Do nothing
