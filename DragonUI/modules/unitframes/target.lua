...
-- ============================================================================
-- NOZDORUI TARGET FRAME MODULE - Optimized for WoW 3.3.5a
-- ============================================================================
...
--  FUNCIГ“N PARA APLICAR POSICIГ“N DESDE WIDGETS (COMO PLAYER.LUA)
local function ApplyWidgetPosition()
    ...
    if widgetConfig then
        ...
        -- TambiГ©n aplicar al frame de Blizzard
        TargetFrame:ClearAllPoints()
        TargetFrame:SetPoint("CENTER", Module.targetFrame, "CENTER", 20, -7)
        
        
    else
        -- Fallback a posiciГіn por defecto
        Module.targetFrame:ClearAllPoints()
...
--  FUNCIГ“N PARA VERIFICAR SI EL TARGET FRAME DEBE ESTAR VISIBLE
...
local function ShowTargetFrameTest()
    --  SISTEMA SIMPLE: Solo llamar al mГ©todo ShowTest del frame
...
local function HideTargetFrameTest()
    --  SISTEMA SIMPLE: Solo llamar al mГ©todo HideTest del frame
...
local function SetupBarHooks()
    ...
    if not TargetFrameHealthBar.NozdorUI_Setup then
        ...
        hooksecurefunc(TargetFrameHealthBar, "SetValue", function(self)
            ...
            --  APLICAR CLASS COLOR SI ESTГЃ HABILITADO
        ...
        TargetFrameHealthBar.NozdorUI_Setup = true
    end

    -- Setup power bar hooks ONCE (sin cambios)
    if not TargetFrameManaBar.NozdorUI_Setup then
        ...
        hooksecurefunc(TargetFrameManaBar, "SetValue", function(self)
            ...
            -- ELIMINAR THROTTLING: ActualizaciГіn inmediata para formas de druida
            -- local now = GetTime()
        ...
        TargetFrameManaBar.NozdorUI_Setup = true
    end
end
...
local function UpdateThreat()
    -- CRITICAL: Check if numeric threat is enabled
    local config = GetConfig()
    if not config or not config.enableNumericThreat then
...
local function UpdateNameBackground()
    ...
    -- LГ“GICA CORRECTA: Verificar tap-denied PRIMERO
    if UnitIsTapped("target") and not UnitIsTappedByPlayer("target") then
        -- Target estГЎ tapped por otro jugador/grupo = GRIS
        r, g, b = 0.5, 0.5, 0.5
    else
        -- Target no estГЎ tap-denied = usar color normal de facciГіn
...
local function InitializeFrame()
    ...
    if not Module.targetFrame then
        Module.targetFrame = addon.CreateUIFrame(200, 75, "target")
        ...
        addon:RegisterEditableFrame({
            ...
            onHide = function()
                ApplyWidgetPosition() -- Aplicar nueva configuraciГіn al salir del editor
            end,
            module = Module
        })
    ...
    if not frameElements.background then
        frameElements.background = TargetFrame:CreateTexture("NozdorUI_TargetBG", "BACKGROUND", nil, -7)
    ...
    if not frameElements.border then
        frameElements.border = TargetFrame:CreateTexture("NozdorUI_TargetBorder", "OVERLAY", nil, 5)
    ...
    local function TargetFrame_CheckClassification_Hook(self, forceNormalTexture)
        -- DespuГ©s de que Blizzard haga su lГіgica, aplicamos la nuestra
    ...
    -- Hook la funciГіn que resetea el threat indicator
    ...
    if not frameElements.elite then
        frameElements.elite = TargetFrame:CreateTexture("NozdorUI_TargetElite", "OVERLAY", nil, 7)
    ...
    if not frameElements.threatNumeric then
        local numeric = CreateFrame("Frame", "NozdorUITargetNumericalThreat", TargetFrame)
    ...
    TargetFrame:SetScale(generalScale * individualScale)

    --  APLICAR POSICIГ“N DESDE WIDGETS SIEMPRE
    ApplyWidgetPosition()

    Module.configured = true
    --  HOOK CRГЌTICO: Proteger contra resets de Blizzard (SIN C_Timer)
    if not Module.classificationHooked then
        -- Hook la funciГіn que Blizzard usa para cambiar clasificaciones
        if _G.TargetFrame_CheckClassification then
            hooksecurefunc("TargetFrame_CheckClassification", function()
                --  SIN C_Timer - Usar frame con OnUpdate para delay mГ­nimo
    ...
    --  MГ‰TODOS ShowTest Y HideTest EXACTAMENTE COMO RETAILUI
    if not TargetFrame.ShowTest then
        TargetFrame.ShowTest = function(self)
            ...
            --  ASEGURAR QUE EL TARGETFRAME ESTГ‰ EN STRATA BAJO PARA QUE EL EDITOR ESTГ‰ ENCIMA
            self:SetFrameStrata("MEDIUM")
            self:SetFrameLevel(10) -- Nivel bajo para que el frame verde estГ© encima
            
            --  ASEGURAR QUE NUESTRAS TEXTURAS PERSONALIZADAS ESTГ‰N VISIBLES
            ...
            --  MOSTRAR DECORACIГ“N ELITE SI EL PLAYER ES ESPECIAL
            if frameElements.elite then
                local classification = UnitClassification("player")
                local name = UnitName("player")
                local coords = nil
                
                --  VERIFICAR SI EL PLAYER ES FAMOSO O TIENE CLASIFICACIГ“N ESPECIAL
                if name and FAMOUS_NPCS[name] then
                    coords = BOSS_COORDS.elite
...
local function OnEvent(self, event, ...)
    ...
    elseif event == "PLAYER_ENTERING_WORLD" then
        ...
        --  CONFIGURAR TEXT SYSTEM AQUГЌ PARA ASEGURAR QUE ESTГ‰ DISPONIBLE
    ...
    if unit == "target" and UnitExists("target") then
        UpdateClassification()
        UpdateTargetHealthBarColor() --  ACTUALIZAR COLOR TAMBIГ‰N
        if Module.textSystem then
            Module.textSystem.update()
        end
    end
    ...
    elseif event == "UNIT_MODEL_CHANGED" or event == "UNIT_DISPLAYPOWER" or event == "UNIT_LEVEL" or event ==
        ...
        local unit = ...
        if unit == "target" and UnitExists("target") then
            --  SIN C_Timer - ActualizaciГіn directa
            UpdateClassification()
        end
...
if not Module.eventsFrame then
    ...
    --  EVENTOS CRГЌTICOS PARA EL TEXT SYSTEM
...
local function RefreshFrame()
    ...
    --  APLICAR CONFIGURACIГ"N INMEDIATAMENTE (incluyendo scale)
    local config = GetConfig()
    
    --  APLICAR SCALE INMEDIATAMENTE (общий масштаб * индивидуальный масштаб)
    local generalScale = (addon.db and addon.db.profile and addon.db.profile.unitframe and addon.db.profile.unitframe.scale) or 1
    local individualScale = config.scale or 1
    TargetFrame:SetScale(generalScale * individualScale)
    
    --  APLICAR POSICIГ“N DESDE WIDGETS INMEDIATAMENTE
    ApplyWidgetPosition()
...
addon.TargetFrame = {
    ...
    UpdateTargetHealthBarColor = UpdateTargetHealthBarColorPublic  --  NUEVA FUNCIГ“N PГљBLICA
}
...
--  HOOK AUTOMГЃTICO PARA CLASS COLOR (compatible con Ace3)
local function SetupTargetClassColorHooks()
    if not _G.NozdorUI_TargetHealthHookSetup then
        ...
        _G.NozdorUI_TargetHealthHookSetup = true
