...
-- ===============================================================
-- NOZDORUI TEXT SYSTEM - SISTEMA HÍBRIDO MEJORADO
-- ===============================================================
...
function TextSystem.GetFrameTextConfig(frameType, configKey)
    ...
    -- Для player frame может использоваться healthFormat/manaFormat вместо textFormat
    local textFormat = config.textFormat or config.healthFormat or "both"
    return {
        textFormat = textFormat,
        breakUpLargeNumbers = config.breakUpLargeNumbers or false,
        showHealthTextAlways = config.showHealthTextAlways ~= nil and config.showHealthTextAlways or false,
        showManaTextAlways = config.showManaTextAlways ~= nil and config.showManaTextAlways or false
    }
end
...
function TextSystem.HookStatusBar(statusBar, parentFrame, prefix, frameType, unit, updateCallback)
    ...
    if not statusBar.NozdorUIHooked then
        statusBar.NozdorUIOriginalSetValue = statusBar.SetValue
        statusBar.SetValue = function(self, value)
            -- Llamar función original
            statusBar.NozdorUIOriginalSetValue(self, value)
...
function TextSystem.SetupHoverEvents(parentFrame, healthBar, manaBar, updateCallback)
    if healthBar then
        ...
        parentFrame.NozdorUIHealthHover = healthHover
    end
    ...
    if manaBar then
        ...
        parentFrame.NozdorUIManaHover = manaHover
    end
end
