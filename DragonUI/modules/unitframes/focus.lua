...
-- ============================================================================
-- NOZDORUI FOCUS FRAME MODULE - WoW 3.3.5a
-- ============================================================================
...
local function SetupBarHooks()
    -- Health bar hooks
    if not FocusFrameHealthBar.NozdorUI_Setup then
    ...
    if config.classcolor and UnitIsPlayer("focus") then
        ...
        FocusFrameHealthBar.NozdorUI_Setup = true
    end
    
    -- Power bar hooks
    if not FocusFrameManaBar.NozdorUI_Setup then
        ...
        FocusFrameManaBar.NozdorUI_Setup = true
    end
end
...
local function InitializeFrame()
    ...
    if not Module.focusFrame then
        Module.focusFrame = addon.CreateUIFrame(180, 70, "focus")
    ...
    if not frameElements.background then
        frameElements.background = FocusFrame:CreateTexture("NozdorUI_FocusBG", "BACKGROUND", nil, -7)
    ...
    if not frameElements.border then
        frameElements.border = FocusFrame:CreateTexture("NozdorUI_FocusBorder", "OVERLAY", nil, 5)
    ...
    if not frameElements.elite then
        frameElements.elite = FocusFrame:CreateTexture("NozdorUI_FocusElite", "OVERLAY", nil, 7)
    ...
    if not frameElements.threatNumeric then
        local numeric = CreateFrame("Frame", "NozdorUIFocusNumericalThreat", FocusFrame)
    ...
    FocusFrame:SetScale(generalScale * individualScale)
    ...
    if not Module.scaleHooked then
        ...
        FocusFrame.SetScale = function(self, scale)
            ...
            local generalScale = (addon.db and addon.db.profile and addon.db.profile.unitframe and addon.db.profile.unitframe.scale) or 1
            local individualScale = config.scale or 1
            local correctScale = generalScale * individualScale
...
local function RefreshFrame()
    ...
    -- Apply scale immediately (общий масштаб * индивидуальный масштаб)
    local generalScale = (addon.db and addon.db.profile and addon.db.profile.unitframe and addon.db.profile.unitframe.scale) or 1
    local individualScale = config.scale or 1
    FocusFrame:SetScale(generalScale * individualScale)
