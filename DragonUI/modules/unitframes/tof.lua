...
-- ============================================================================
-- NOZDORUI FOCUS OF TARGET FRAME MODULE - WoW 3.3.5a
...
local function ApplyWidgetPosition()
    ...
    local widgetConfig = addon.db and addon.db.profile.widgets and addon.db.profile.widgets.tof
    
    if widgetConfig then
        Module.totFrame:ClearAllPoints()
        local anchorFrame = _G[widgetConfig.anchorFrame] or FocusFrame or UIParent
        Module.totFrame:SetPoint(widgetConfig.anchor or "BOTTOMRIGHT", anchorFrame, widgetConfig.anchorParent or "BOTTOMRIGHT", 
                               widgetConfig.posX or -8, widgetConfig.posY or -30)
        
        -- También aplicar al frame de Blizzard
        if FocusFrameToT then
            FocusFrameToT:ClearAllPoints()
            FocusFrameToT:SetPoint("CENTER", Module.totFrame, "CENTER", 0, 0)
        end
    else
        -- Fallback a posición por defecto
        Module.totFrame:ClearAllPoints()
        local anchorFrame = FocusFrame or UIParent
        Module.totFrame:SetPoint("BOTTOMRIGHT", anchorFrame, "BOTTOMRIGHT", -8, -30)
        if FocusFrameToT then
            FocusFrameToT:ClearAllPoints()
            FocusFrameToT:SetPoint("CENTER", Module.totFrame, "CENTER", 0, 0)
        end
...
--  FUNCIÓN PARA VERIFICAR SI EL TOF FRAME DEBE ESTAR VISIBLE
local function ShouldToFFrameBeVisible()
    return UnitExists("focus") and UnitExists("focustarget")
end

--  FUNCIONES DE TESTEO
local function ShowToFFrameTest()
    -- Mostrar el frame aunque no haya focustarget
    if FocusFrameToT then
        FocusFrameToT:Show()
    end
end

local function HideToFFrameTest()
...
local function SetupBarHooks()
    ...
    if not FocusFrameToTHealthBar.NozdorUI_Setup then
        ...
        FocusFrameToTHealthBar.NozdorUI_Setup = true
    end

    -- Power bar hooks (IGUAL que ToT pero para "focustarget")
    if not FocusFrameToTManaBar.NozdorUI_Setup then
        ...
        FocusFrameToTManaBar.NozdorUI_Setup = true
    end
end
...
local function InitializeFrame()
    ...
    if Module.totFrame then
        ...
        -- Position FocusFrameToT relative to anchor
        FocusFrameToT:ClearAllPoints()
        FocusFrameToT:SetPoint("CENTER", Module.totFrame, "CENTER", 0, 0)
    else
    ...
    FocusFrameToT:SetScale(generalScale * individualScale)
    ...
    if not frameElements.background then
        frameElements.background = FocusFrameToT:CreateTexture("NozdorUI_FoTBG", "BACKGROUND", nil, 0)
    ...
    if not frameElements.border then
        frameElements.border = FocusFrameToTHealthBar:CreateTexture("NozdorUI_FoTBorder", "OVERLAY", nil, 1)
    ...
    if not frameElements.elite then
        local eliteFrame = CreateFrame("Frame", "NozdorUI_FoTEliteFrame", FocusFrameToT)
        eliteFrame:SetFrameStrata("MEDIUM")
        eliteFrame:SetAllPoints(FocusFrameToTPortrait)

        frameElements.elite = eliteFrame:CreateTexture("NozdorUI_FoTElite", "OVERLAY", nil, 1)
        frameElements.elite:SetTexture(TEXTURES.BOSS)
        frameElements.elite:Hide()
    end
...
local function OnEvent(self, event, ...)
    if event == "ADDON_LOADED" then
        ...
        if name == "NozdorUI" and not Module.initialized then
            ...
            Module.totFrame = addon.CreateUIFrame(120, 47, "tof")
            
            --  PERSONALIZAR TEXTO PARA TOF
            if Module.totFrame.editorText then
                Module.totFrame.editorText:SetText("Цель фокуса")
            end
            ...
            addon:RegisterEditableFrame({
                ...
                blizzardFrame = FocusFrameToT,
                configPath = {"widgets", "tof"},
                hasTarget = ShouldToFFrameBeVisible,
                showTest = ShowToFFrameTest,
                hideTest = HideToFFrameTest,
                onHide = function()
                    ApplyWidgetPosition()
                end,
                module = Module
...
local function ResetFrame()
    ...
    local defaults = addon.defaults and addon.defaults.profile.widgets.tof
    if defaults then
        addon.db.profile.widgets.tof = {
            anchor = defaults.anchor or "BOTTOMRIGHT",
            anchorParent = defaults.anchorParent or "BOTTOMRIGHT",
            anchorFrame = defaults.anchorFrame or "FocusFrame",
            posX = defaults.posX or -8,
            posY = defaults.posY or -30
        }
    else
        addon.db.profile.widgets.tof = {
            anchor = "BOTTOMRIGHT",
