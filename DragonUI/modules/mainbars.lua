...
local function InitializeMainbars()
   ...
   function MainMenuBarMixin:actionbutton_setup()
    ...
    function addon.AdjustArtFrameSize()
        if not pUiMainBar then
            return
        end
        
        local barWidth = pUiMainBar:GetWidth()
        local barHeight = pUiMainBar:GetHeight()
        
        if barWidth and barHeight and barWidth > 0 and barHeight > 0 then
            if pUiMainBar.BorderArt then
                local borderArt = pUiMainBar.BorderArt
                
                borderArt:ClearAllPoints()
                borderArt:SetPoint("TOPLEFT", pUiMainBar, "TOPLEFT", -2 , 2)
                borderArt:SetPoint("BOTTOMRIGHT", pUiMainBar, "BOTTOMRIGHT", -6 , -6)
            end
            
            if MainMenuBarArtFrame then
                MainMenuBarArtFrame:SetParent(pUiMainBarArt)
                local artWidth = barWidth - 40
                local artHeight = barHeight + 10
                MainMenuBarArtFrame:ClearAllPoints()
                MainMenuBarArtFrame:SetPoint("CENTER", pUiMainBar, "CENTER", -15, 0)
                MainMenuBarArtFrame:SetSize(artWidth, artHeight)
            end
            
            if pUiMainBar.Background then
                local background = pUiMainBar.Background
                background:ClearAllPoints()
                background:SetPoint("TOPLEFT", pUiMainBar, "TOPLEFT", 4, 1)
                background:SetPoint("BOTTOMRIGHT", pUiMainBar, "BOTTOMRIGHT", -10, -5)
            end
    end
end
    ...
    function MainMenuBarMixin:actionbar_art_setup()
        ...
        addon.AdjustArtFrameSize()
        
        local adjustFrame = CreateFrame("Frame")
        local totalElapsed = 0
        adjustFrame:SetScript("OnUpdate", function(self, elapsed)
        ...
        UpdateGryphonStyle()
        local adjustFrame2 = CreateFrame("Frame")
        adjustFrame2:SetScript("OnUpdate", function(self, elapsed)
            self.elapsed = (self.elapsed or 0) + elapsed
    ...
    function MainMenuBarMixin:actionbar_setup()
        ...
        local pagesShow = (buttonsConfig and buttonsConfig.pages and buttonsConfig.pages.show) or (config and config.buttons and config.buttons.pages and config.buttons.pages.show)
        if pagesShow then
            ...
            local pagesFont = (buttonsConfig and buttonsConfig.pages and buttonsConfig.pages.font) or (config and config.buttons and config.buttons.pages and config.buttons.pages.font)
            if pagesFont then
   ...
   -- Función específica para deshabilitar MainMenuBarMaxLevelBar
    ...
    local function ApplyNozdorUIExpRepBarStyling()
    ...
    initFrame:SetScript("OnEvent", function(self, event, addonName)
        ...
        elseif event == "PLAYER_LOGIN" then
            -- Profile callbacks removed - using C_CacheSystem instead of AceDB
            -- Changes are automatically saved via SaveNozdorUIToCache()
                    -- Initial refresh
...
function addon.UpdateGryphonStyle()
    ...
    if db_style.gryphons == 'old' then
        ...
        MainMenuBarRightEndCap:SetClearPoint('BOTTOMRIGHT', 74, -22)
    ...
    elseif db_style.gryphons == 'new' then
        MainMenuBarLeftEndCap:SetClearPoint('BOTTOMLEFT', -94, -23)  -- Было -94, стало -84 (ближе к центру на 10)
        MainMenuBarRightEndCap:SetClearPoint('BOTTOMRIGHT', 85, -23)  -- Было 95, стало 85 (ближе к центру на 10)
    ...
    elseif db_style.gryphons == 'flying' then
        MainMenuBarLeftEndCap:SetClearPoint('BOTTOMLEFT', -80, -21)  -- Было -80, стало -70 (ближе к центру на 10)
        MainMenuBarRightEndCap:SetClearPoint('BOTTOMRIGHT', 70, -21)  -- Было 80, стало 70 (ближе к центру на 10)
...
function addon.RefreshMainbarsSystem()
    ...
    local buttonsConfig = addon.db and addon.db.profile and addon.db.profile.buttons
    if buttonsConfig and buttonsConfig.pages and pUiMainBarArt and ActionBarUpButton and ActionBarDownButton then
        if buttonsConfig.pages.show then
            if do_action and do_action.SetNumPagesButton then
                do_action.SetNumPagesButton(ActionBarUpButton, pUiMainBarArt, 'pageuparrow', 8)
                do_action.SetNumPagesButton(ActionBarDownButton, pUiMainBarArt, 'pagedownarrow', -14)
            end
            ...
            if MainMenuBarPageNumber then
                MainMenuBarPageNumber:SetParent(pUiMainBarArt)
                MainMenuBarPageNumber:SetClearPoint('CENTER', ActionBarDownButton, -1, 12)
                local pagesFont = buttonsConfig.pages.font or (config and config.buttons and config.buttons.pages and config.buttons.pages.font)
                if pagesFont then
                    MainMenuBarPageNumber:SetFont(pagesFont[1], pagesFont[2], pagesFont[3])
                end
                MainMenuBarPageNumber:SetShadowColor(0, 0, 0, 1)
