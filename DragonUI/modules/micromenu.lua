...
-- CRITICAL: Include all possible buttons, including conditional ones
-- Order matches sample: Character, Spellbook, Talent, Achievement, QuestLog, Socials, PVP, LFD, EncounterJournal, Store, Collections, MainMenu, Help
local function BuildMicroButtonsList()
    local buttons = {}
    
    -- Always include these buttons in correct order
    local alwaysButtons = {
        ...
        _G.PVPMicroButton,  -- PVP before LFD
        _G.LFDMicroButton,
    }
    
    for _, btn in ipairs(alwaysButtons) do
        if btn then
    ...
    if isAscensionServer then
        -- Ascension-specific buttons (insert after LFD, before MainMenu)
        if _G.PathToAscensionMicroButton then
            table.insert(buttons, _G.PathToAscensionMicroButton)
        end
        if _G.ChallengesMicroButton then
            table.insert(buttons, _G.ChallengesMicroButton)
        end
    else
        -- Standard server buttons (insert after LFD, before MainMenu)
        -- Order: EncounterJournal, Store, Collections
        if _G.EncounterJournalMicroButton then
            table.insert(buttons, _G.EncounterJournalMicroButton)
        end
        if _G.StoreMicroButton then
            table.insert(buttons, _G.StoreMicroButton)
        end
        if _G.CollectionsMicroButton then
            table.insert(buttons, _G.CollectionsMicroButton)
        end
    end
    
    -- Add MainMenu and Help at the end
    if _G.MainMenuMicroButton then
        table.insert(buttons, _G.MainMenuMicroButton)
    end
    if _G.HelpMicroButton then
        table.insert(buttons, _G.HelpMicroButton)
    end
    
    return buttons
end

local MICRO_BUTTONS = BuildMicroButtonsList()
...
local function GetAtlasKey(buttonName)
    ...
    if isAscensionServer then
        buttonMap = {
            ...
            mainmenu = "UI-HUD-MicroMenu-GameMenu", -- Use Help icon instead of Shop
        ...
        buttonMap = {
            ...
            encounterjournal = "UI-HUD-MicroMenu-EncounterJournal",
...
local function UpdateCharacterPortraitVisibility()
    if MicroButtonPortrait then
        -- Always show portrait for colored icons
...
local function ApplyMicromenuSystem()
    ...
    local function HideUnwantedBagFrames()
        ...
        if KeyRingButton then
            ...
            -- CRITICAL: Also hide border textures via GetRegions (like bags)
            ...
            for j = 1, numRegions do
                ...
                if region and region:GetObjectType() == "Texture" then
                    ...
                    if texture then
                        ...
                        -- Get icon texture for comparison
                        local iconTexture = KeyRingButtonIconTexture
                        local isIcon = (region == iconTexture)
                        
                        -- Hide square borders and backgrounds, but keep icon
                        if not isIcon and (textureLower:find("border") or textureLower:find("background") or 
                            textureLower:find("frame") or textureLower:find("square") or
                            regionNameLower:find("border") or regionNameLower:find("background") or
                            (drawLayer and (drawLayer == "BACKGROUND" or drawLayer == "BORDER"))) then
                            region:Hide()
                            if region.SetAlpha then
                                region:SetAlpha(0)
                            end
    ...
    local function SetupPVPButton(button)
        ...
        if not button.DragonUIBackground then
            ...
            local dx, dy = -1, -1  -- Fixed: Changed from dy = 1 to dy = -1 to match other buttons
    ...
    function MainMenuMicroButtonMixin:bagbuttons_setup()
        ...
        -- CRITICAL: Hide unwanted border textures on KeyRingButton
        HideUnwantedBagFrames()
        ...
        if not pUiBagsBar.registeredInEditor then
            ...
            local bagsFrame = addon.CreateUIFrame(210, 50, "Панель сумок")

            -- CRITICAL: Apply scale from config
            local bagsScaleConfig = addon.db and addon.db.profile and addon.db.profile.bags
            local bagsScale = bagsScaleConfig and bagsScaleConfig.scale or 0.9
            bagsFrame:SetScale(bagsScale)
            if _G.pUiBagsBar then
                _G.pUiBagsBar:SetScale(bagsScale)
            end
    ...
    local function setupMicroButtons(xOffset)
        -- Rebuild button list to ensure we have all current buttons
        MICRO_BUTTONS = BuildMicroButtonsList()
        
        local buttonxOffset = 0

        -- Always use normal (colored) icons mode
        local config = addon.db.profile.micromenu.normal
        ...
        if not menu.registeredInEditor then
            ...
            local microMenuFrame = addon.CreateUIFrame(250, 40, "Микроменю") -- Increased from 240 to 250 to cover all buttons
            ...
            -- Function to update all button positions (stored on menu for later access)
            menu.UpdateAllButtonPositions = function()
                ...
                local buttonxOffset = 0
                for _, button in pairs(MICRO_BUTTONS) do
                    if button and button:GetParent() == menu then
                        -- Temporarily restore SetPoint to allow position update
                        if button._dragonUIOriginalSetPoint then
                            button.SetPoint = button._dragonUIOriginalSetPoint
                        end
                        
                        button:ClearAllPoints()
                        button:SetPoint('BOTTOMLEFT', menu, 'BOTTOMRIGHT', buttonxOffset, 55)
                        
                        -- Re-apply SetPoint hook
                        if button._dragonUIOriginalSetPoint then
                            button.SetPoint = function(self, point, relativeTo, relativePoint, x, y)
                                -- Only allow if it's our menu
                                if relativeTo == menu or relativeTo == nil then
                                    if self._dragonUIOriginalSetPoint then
                                        self._dragonUIOriginalSetPoint(self, point, relativeTo, relativePoint, x, y)
                                    end
                                end
                            end
                        end
            ...
            local UpdateAllButtonPositions = menu.UpdateAllButtonPositions

            -- Track if container is being dragged
            local isDragging = false

            -- Hook for the menu to follow the container when moved
            microMenuFrame:HookScript("OnDragStart", function(self)
                isDragging = true
                -- Update button positions when dragging starts
            ...
            microMenuFrame:HookScript("OnDragStop", function(self)
                ...
                -- Update all button positions after container stops moving
                UpdateAllButtonPositions()
            end)
            ...
            microMenuFrame:HookScript("OnShow", function(self)
                menu:ClearAllPoints()
                menu:SetPoint("CENTER", self, "CENTER", menuXOffset, -70) -- Use variable
                -- Update button positions when container is shown
                UpdateAllButtonPositions()
            end)

            -- Continuous hook to maintain position and update buttons during drag
            microMenuFrame:HookScript("OnUpdate", function(self)
                ...
                elseif isDragging then
                    ...
                    menu:ClearAllPoints()
                    menu:SetPoint("CENTER", self, "CENTER", menuXOffset, -70)
                    -- Update button positions to follow menu
                    UpdateAllButtonPositions()
                end
            end)
            ...
            -- Container already exists, update menu position relative to container
            local frameInfo = addon:GetEditableFrameInfo("micromenu")
            if frameInfo and frameInfo.frame then
                local microMenuFrame = frameInfo.frame
                local menuXOffset = isAscensionServer and -170 or -140
                ...
                -- Update menu position relative to container
                menu:ClearAllPoints()
                menu:SetPoint("CENTER", microMenuFrame, "CENTER", menuXOffset, -70)
                
                -- Update button positions
                if menu.UpdateAllButtonPositions then
                    menu:UpdateAllButtonPositions()
                end
            else
                -- Fallback: set position relative to UIParent if container not found
                menu:ClearAllPoints()
                menu:SetPoint('BOTTOMLEFT', UIParent, 'BOTTOMRIGHT', xPosition, yPosition)
            end
        end
        ...
        for _, button in pairs(MICRO_BUTTONS) do
            if button then
                ...
                -- CRITICAL: Check if this is a custom button BEFORE texture_strip
                local isCustomButton = (buttonName == "EncounterJournal" or buttonName == "Store" or buttonName == "Collections")
                
                -- Only strip textures for non-custom buttons
                if not isCustomButton then
                ...
                -- CRITICAL: Unhide from MainMenuBarArtFrame first (if it exists)
                local originalParent = button:GetParent()
                if originalParent == _G.MainMenuBarArtFrame then
                    -- Button is still attached to MainMenuBarArtFrame, unhide it first
                    if _G.MainMenuBarArtFrame then
                        _G.MainMenuBarArtFrame:Show()
                        _G.MainMenuBarArtFrame:SetAlpha(1)
                    end
                end

                -- CRITICAL: Store reference to menu and hook SetParent to prevent buttons from being moved back
                if not button._dragonUIOriginalSetParent then
                    button._dragonUIMenu = menu
                    button._dragonUIOriginalSetParent = button.SetParent
                    
                    -- CRITICAL: Hook SetParent to prevent buttons from being moved back to MainMenuBarArtFrame
                    button.SetParent = function(self, newParent)
                        -- Block any attempts to set parent back to MainMenuBarArtFrame
                        if newParent == _G.MainMenuBarArtFrame then
                            -- Force parent to be menu instead
                            button._dragonUIOriginalSetParent(self, button._dragonUIMenu)
                        else
                            -- Allow other parent changes
                            button._dragonUIOriginalSetParent(self, newParent)
                        end
                    end
                end
                
                -- CRITICAL: Check current parent and force change if needed
                local currentParent = button:GetParent()
                
                -- CRITICAL: If button is on MainMenuBarArtFrame, we MUST unhide it first
                if currentParent == _G.MainMenuBarArtFrame then
                    -- Button is still on MainMenuBarArtFrame, unhide it first
                    if _G.MainMenuBarArtFrame then
                        _G.MainMenuBarArtFrame:Show()
                        _G.MainMenuBarArtFrame:SetAlpha(1)
                        -- Also unhide button while still on MainMenuBarArtFrame
                        button:Show()
                        button:SetAlpha(1)
                    end
                    
                    -- CRITICAL: Force remove from MainMenuBarArtFrame by using original SetParent
                    button:ClearAllPoints()
                    -- Use original SetParent to bypass the hook temporarily
                    button._dragonUIOriginalSetParent(button, nil)
                    -- Now set to menu using the hook (hook will prevent going back to MainMenuBarArtFrame)
                    button:SetParent(menu)
                elseif currentParent ~= menu then
                    -- Already removed or on different parent, just set (hook will prevent MainMenuBarArtFrame)
                    button:ClearAllPoints()
                    button:SetParent(menu)
                end
                
                -- CRITICAL: Verify parent was actually changed
                if button:GetParent() == _G.MainMenuBarArtFrame then
                    -- Still on MainMenuBarArtFrame - this shouldn't happen with hook, but try again
                    if _G.MainMenuBarArtFrame then
                        _G.MainMenuBarArtFrame:Show()
                        _G.MainMenuBarArtFrame:SetAlpha(1)
                        button:Show()
                        button:SetAlpha(1)
                        button:ClearAllPoints()
                        -- Force using original SetParent
                        button._dragonUIOriginalSetParent(button, nil)
                        button:SetParent(menu)
                    end
                end
                
                -- CRITICAL: Ensure button is visible and enabled before positioning
                button:Show()
                button:SetAlpha(1)
                button:SetFrameStrata('MEDIUM')
                button:SetFrameLevel(menu:GetFrameLevel() + 1)
                
                -- CRITICAL: Final verification - if still on wrong parent, force change
                if button:GetParent() ~= menu then
                    -- Force set again with more aggressive approach
                    button:ClearAllPoints()
                    button._dragonUIOriginalSetParent(button, nil)
                    button:SetParent(menu)
                    button:Show()
                ...
                -- Always use colored icons size
                button:SetSize(32, 40)

                -- CRITICAL: Ensure parent is set correctly (check again after size change)
                if button:GetParent() ~= menu then
                    button:SetParent(menu)
                end

                -- CRITICAL: Clear all points and set new position
                button:ClearAllPoints()
                button:SetPoint('BOTTOMLEFT', menu, 'BOTTOMRIGHT', buttonxOffset, 55)
                
                -- CRITICAL: Block further position changes AFTER setting position
                if not button._dragonUIOriginalSetPoint then
                    button._dragonUIOriginalSetPoint = button.SetPoint
                end
                button.SetPoint = function(self, point, relativeTo, relativePoint, x, y)
                    -- Only allow if it's our menu
                    if relativeTo == menu or relativeTo == nil then
                ...
                -- CRITICAL: Force show button regardless of original state
                button:Show()
                button:SetAlpha(1)
                button:SetFrameStrata('MEDIUM')
                button:SetFrameLevel(menu:GetFrameLevel() + 1)
                
                -- CRITICAL: Final verification - check parent and visibility
                if button:GetParent() ~= menu then
                    -- Parent was changed back, force set again
                    button:SetParent(menu)
                    button:Show()
                    button:SetAlpha(1)
                end
                
                -- CRITICAL: If still not visible, unhide parent
                if not button:IsVisible() then
                    -- Try to unhide MainMenuBarArtFrame temporarily
                    if _G.MainMenuBarArtFrame then
                        _G.MainMenuBarArtFrame:Show()
                        _G.MainMenuBarArtFrame:SetAlpha(1)
                    end
                    -- Force parent change again
                    button:SetParent(menu)
                    button:Show()
                    button:SetAlpha(1)
                end
                ...
                -- CRITICAL: Check if this is a custom button that might not have atlas textures
                local upCoords = not isCharacterButton and not isPVPButton and not isCustomButton and GetColoredTextureCoords(name, "Up") or nil
                if buttonName == "Store" then
                    upCoords = GetColoredTextureCoords("store", "Up")
                end
                if buttonName == "EncounterJournal" then
                    upCoords = GetColoredTextureCoords("encounterjournal", "Up")
                end
                -- Always use colored icons
                if isPVPButton then
                    SetupPVPButton(button)
                elseif isCharacterButton then
                    SetupCharacterButton(button)
                else
                    -- Colored icons or custom buttons
                    local microTexture = 'Interface\\AddOns\\DragonUI\\Textures\\Micromenu\\uimicromenu2x'
                    ...
                    -- Handle Store button special case
                    if buttonName == "Store" then
                        downCoords = GetColoredTextureCoords("store", "Down")
                        disabledCoords = GetColoredTextureCoords("store", "Disabled")
                        mouseoverCoords = GetColoredTextureCoords("store", "Mouseover")
                    end
                    
                    -- Handle EncounterJournal button special case
                    if buttonName == "EncounterJournal" then
                        downCoords = GetColoredTextureCoords("encounterjournal", "Down")
                        disabledCoords = GetColoredTextureCoords("encounterjournal", "Disabled")
                        mouseoverCoords = GetColoredTextureCoords("encounterjournal", "Mouseover")
                    end
                    ...
                    -- Add background for all buttons (including Store and EncounterJournal)
                    -- Only skip Collections if it has its own textures
                    local shouldAddBackground = true
                    if buttonName == "Collections" then
                        -- Collections might have its own textures, check if we should skip
                        shouldAddBackground = false
                    end
                    
                    if shouldAddBackground and not button.DragonUIBackground then
                        local backgroundTexture = 'Interface\\AddOns\\DragonUI\\Textures\\Micromenu\\uimicromenu2x'
                        local dx, dy = -1, -1
                        local offX, offY = button:GetPushedTextOffset()
                        local sizeX, sizeY = button:GetSize()
    ...
    if frameInfo and frameInfo.frame then
        ...
        -- Update all button positions after menu position is updated
        if _G.pUiMicroMenu.UpdateAllButtonPositions then
            _G.pUiMicroMenu:UpdateAllButtonPositions()
        else
            updateMicroButtonSpacing()
        end
    else
    ...
    function addon.RefreshBagsPosition()
        ...
        -- CRITICAL: Get bags scale from config
        local bagsConfig = addon.db and addon.db.profile and addon.db.profile.bags
        local bagsScale = bagsConfig and bagsConfig.scale or 0.9

        local frameInfo = addon:GetEditableFrameInfo("bagsbar")
        if frameInfo and frameInfo.frame then
            -- Aplicar posición desde database o usar default
            local widgetConfig = addon.db and addon.db.profile.widgets and addon.db.profile.widgets.bagsbar
            if widgetConfig and widgetConfig.anchor then
                frameInfo.frame:ClearAllPoints()
                frameInfo.frame:SetPoint(widgetConfig.anchor or "BOTTOMRIGHT", UIParent,
                    widgetConfig.anchor or "BOTTOMRIGHT", widgetConfig.posX or 1, widgetConfig.posY or 41)
            else
                frameInfo.frame:ClearAllPoints()
                frameInfo.frame:SetPoint("BOTTOMRIGHT", UIParent, "BOTTOMRIGHT", 1, 41)
            end

            -- CRITICAL: Apply scale to the container frame
            frameInfo.frame:SetScale(bagsScale)
            
            -- Also apply scale to pUiBagsBar if it exists
            if _G.pUiBagsBar then
                _G.pUiBagsBar:SetScale(bagsScale)
            end
    ...
    function addon.RefreshBagsVehicle()
        ...
        if addon.db.profile.micromenu.hide_on_vehicle then
            ...
            -- CRITICAL: Also hide MainMenuBarBackpackButton in vehicle
            if MainMenuBarBackpackButton then
                RegisterStateDriver(MainMenuBarBackpackButton, 'visibility', '[vehicleui] hide;show')
            end
        else
            UnregisterStateDriver(_G.pUiBagsBar, 'visibility')
            
            -- CRITICAL: Unregister MainMenuBarBackpackButton state driver
            if MainMenuBarBackpackButton then
                UnregisterStateDriver(MainMenuBarBackpackButton, 'visibility')
            end
        end
    end
    ...
    for _, button in pairs(MICRO_BUTTONS) do
        if button then
            ...
            if button.SetPoint == addon._noop or (button._dragonUIOriginalSetPoint and button.SetPoint ~= button._dragonUIOriginalSetPoint) then
                if button._dragonUIOriginalSetPoint then
                    button.SetPoint = button._dragonUIOriginalSetPoint
                else
            ...
            -- Re-apply SetPoint hook if needed
            if button._dragonUIOriginalSetPoint and _G.pUiMicroMenu then
                button.SetPoint = function(self, point, relativeTo, relativePoint, x, y)
                    -- Only allow if it's our menu
                    if relativeTo == _G.pUiMicroMenu or relativeTo == nil then
                        if self._dragonUIOriginalSetPoint then
                            self._dragonUIOriginalSetPoint(self, point, relativeTo, relativePoint, x, y)
                        end
                    end
                end
            elseif originalSetPoint == addon._noop then
    ...
    -- CRITICAL: Handle vehicle exit to restore bags state
    addon.package:RegisterEvents(function(self, event, unit)
        if event == 'UNIT_EXITED_VEHICLE' and (unit == 'player' or not unit) then
            -- Restore bags state after exiting vehicle
            C_Timer.After(0.1, function()
                -- CRITICAL: Full refresh to restore all bag states (including collapsed state)
                if addon.RefreshBags then
                    addon.RefreshBags()
                else
                    -- Fallback if RefreshBags is not available
                    if MainMenuMicroButtonMixin and MainMenuMicroButtonMixin.bagbuttons_refresh then
                        MainMenuMicroButtonMixin:bagbuttons_refresh()
                    end
                    if MainMenuMicroButtonMixin and MainMenuMicroButtonMixin.bagbuttons_reposition then
                        MainMenuMicroButtonMixin:bagbuttons_reposition()
                    end
                end
                -- Ensure KeyRingButton is properly restored
                if KeyRingButton then
                    if HasKey() then
                        KeyRingButton:Show()
                    else
    ...
    addon.package:RegisterEvents(function()
        ...
        -- CRITICAL: Refresh position after setup to ensure correct positioning
        ...
        addon.core:ScheduleTimer(function()
            ...
            -- CRITICAL: Refresh position again after delayed setup to ensure correct positioning
            if addon.RefreshMicromenuPosition then
                addon.RefreshMicromenuPosition();
            end
        end, 0.5)
...
local function LoadDefaultWidgetSettings()
    ...
    if not addon.db.profile.widgets.micromenu then
        ...
        -- Always use normal (colored) icons mode
        local config = addon.db.profile.micromenu and addon.db.profile.micromenu.normal
        ...
        if config then
            ...
            addon.db.profile.widgets.micromenu = {
                anchor = "BOTTOMRIGHT",
                posX = -183,  -- Moved left by 17 pixels (was -166)
                posY = 4
            }
        end
    end
