--[[
    Minimap Module - Adaptado de RetailUI
    Código base por Dmitriy (RetailUI) adaptado para NozdorUI
]] -- Get addon reference - either from XML parameter or global
local addon = select(2, ...);

--  Import atlas function for tracking icons
...
local function ReplaceBlizzardFrame(frame)
    ...
    -- Blip texture (configurable: new icons vs old Blizzard icons)
...
local function CreateMinimapBorderFrame(width, height)
    ...
    -- Use throttled OnUpdate to prevent freezes (update every 0.1 seconds instead of every frame)
    local lastUpdate = 0
...
local function ApplyAddonIconSkin(button)
    ...
    -- Primero verificar database (principal)
...
function MinimapModule:ApplyMinimapSystem()
    ...
    -- Store original settings before applying changes
...
function MinimapModule:InitializeMinimapSystem()
    ...
    self.minimapFrame = CreateUIFrame(230, 230, "minimap")
    ...
    -- Hook TimeManagerFrame to reposition it so it doesn't overlap minimap
    local function RepositionTimeManagerFrame()
        if not TimeManagerFrame or not TimeManagerFrame:IsShown() then
            return
        end
        
        -- Get minimap position
        local minimapX, minimapY = self.minimapFrame:GetCenter()
        if not minimapX or not minimapY then
            return
        end
        
        -- Position TimeManagerFrame to the left of minimap, or below if not enough space
        local frameWidth = TimeManagerFrame:GetWidth() or 200
        local frameHeight = TimeManagerFrame:GetHeight() or 200
        local screenWidth = UIParent:GetWidth()
        local screenHeight = UIParent:GetHeight()
        
        -- Try to position to the left of minimap first
        local newX = minimapX - (self.minimapFrame:GetWidth() / 2) - (frameWidth / 2) - 20
        local newY = minimapY
        
        -- If not enough space on the left, position below minimap
        if newX < frameWidth / 2 then
            newX = minimapX
            newY = minimapY - (self.minimapFrame:GetHeight() / 2) - (frameHeight / 2) - 20
        end
        
        -- Ensure frame stays on screen
        newX = math.max(frameWidth / 2, math.min(screenWidth - frameWidth / 2, newX))
        newY = math.max(frameHeight / 2, math.min(screenHeight - frameHeight / 2, newY))
        
        TimeManagerFrame:ClearAllPoints()
        TimeManagerFrame:SetPoint("CENTER", UIParent, "BOTTOMLEFT", newX, newY)
    end
    
    -- Hook TimeManagerFrame Show method
    if TimeManagerFrame then
        hooksecurefunc(TimeManagerFrame, "Show", RepositionTimeManagerFrame)
    else
        -- If TimeManagerFrame doesn't exist yet, hook it after TimeManager loads
        local initFrame = CreateFrame("Frame")
        initFrame:RegisterEvent("ADDON_LOADED")
        initFrame:SetScript("OnEvent", function(self, event, addonName)
            if addonName == "Blizzard_TimeManager" and TimeManagerFrame then
                hooksecurefunc(TimeManagerFrame, "Show", RepositionTimeManagerFrame)
    ...
    -- Also hook the clock button click to reposition after a short delay
    if TimeManagerClockButton then
        TimeManagerClockButton:HookScript("OnClick", function()
            addon.core:ScheduleTimer(RepositionTimeManagerFrame, 0.1)
        end)
    end
...
function MinimapModule:Initialize()
    ...
    if not isEnabled then
        ...
        -- Don't apply any modifications when disabled
        return
    end

    -- Only apply modifications if module is enabled
    self:ApplyMinimapSystem()
end

-- Eliminar las funciones que no existen más
